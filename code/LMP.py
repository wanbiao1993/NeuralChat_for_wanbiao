...
class LMP:
    ...
    def __call__(self, query, context='', **kwargs):
        ...
        while True:
            try:
                # # code_str = openai.completions.create(
                ...
                code_str = self.client.completions.create(prompt=prompt,
                stop=self._stop_tokens,
                temperature=self._cfg['temperature'],
                model=self._cfg['engine'],
                max_tokens=self._cfg['max_tokens']).choices[0].text.strip()
                print(code_str)
                break
...
class LMPFGen:
    ...
    def create_f_from_sig(self, f_name, f_sig, other_vars=None, fix_bugs=False, return_src=False):
        ...
        while True:
            try:
                # f_src = openai.completions.create(
                ...
                #     max_tokens=self._cfg['max_tokens']
                # )['choices'][0]['text'].strip()
                f_src = self.client.completions.create(prompt=prompt,
                stop=self._stop_tokens,
                temperature=self._cfg['temperature'],
                model=self._cfg['engine'],
                max_tokens=self._cfg['max_tokens']).choices[0].text.strip()
        ...
        if fix_bugs:
            ...
            f_src = self.client.edits.create(model='code-davinci-edit-001',
            input='# ' + f_src,
            temperature=0,
